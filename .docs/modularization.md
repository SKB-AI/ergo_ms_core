...
**Общее ядро остаётся в ядре.** Пользователи (`auth_user.public_id`), сессии устройств, роли ADP, меню CMS, аудит, inbox уведомлений принадлежат ядру. Модуль не владеет пользователем: хранит `user_public_id` (UUID), без внешнего ключа на чужие таблицы. Пункты меню пишет ядро в свои таблицы: процесс модуля отдаёт дерево через группу моста `menu.catalog`, кнопка «Восстановить» в CMS забирает его по HTTP. Если папка `modules/<name>/` есть на диске ядра, `restore_menu` по-прежнему может проиграть локальные миграции.
...
| Меню CMS | ядро | клиент через API ядра | процесс модуля → `menu.catalog` → таблицы ядра; запасной вариант — миграции на диске ядра |
